...
**Table S3:** Statistical model output of beta diversity assessments of all ASVs based on (**A**) treatment and (**B**) genotype.
```{r table S3}
...
**Table S4:** Statistical model output of alpha diversity assessments of all ASVs.
```{r table S4}
...
## combine all alpha of all ASVs
simp_noA_sum <- data.frame(metric = c(rep("simpson", 4)), summary(simp_mod_noA)[["coefficients"]]) %>% rename("Pvalue" = Pr...z..)
shan_noA_sum <- data.frame(metric = c(rep("shannon", 4)), summary(shan_mod_noA)[["coefficients"]])[-4] %>% rename("Pvalue" = Pr...t..)
rich_noA_sum <- data.frame(metric = c(rep("richness", 4)), summary(rich_mod_noA)[["coefficients"]])[-4] %>% rename("Pvalue" = Pr...t..)
even_noA_sum <- data.frame(metric = c(rep("evenness", 4)), summary(even_mod_noA)[["coefficients"]]) %>% rename("Pvalue" = Pr...z..)
...
**Table S5:** Non-Aquarickettsia ASV beta (**A-B**) and alpha (**C**) diversity across samples.
```{r render the tables for S5}

## table 5A: ANOVA of non aqua ASVs
...
## table 5B: PERMANOVA of non aqua ASVs
...
## table 5C: all alpha stats of non aqua ASVs
