...
plot_bar(phylo_core_genus_rel, x = "Genotype", fill="Genus", colour="Genus")+
  ...
  scale_colour_manual(values = c('#017a4a','#FFCE4E','#3d98d3','#7559a2','#ff363c','#8cdb5e','#fb8c00','#794924','#d6d6d6'))
...
rel_core_plot <- ggplot(data = pseq_core_rel_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
  ...
  ggtitle("C.   PCoA of core taxa (relative abundance)")
rel_core_plot
...
acc_plot <- ggplot(data = phylo_acc_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
  ...
  ggtitle("C.   PCoA of accessory taxa")
acc_plot
...
rel_acc_plot <- ggplot(data = phylo_acc_rel_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
  ...
  ggtitle("C.   PCoA of accessory taxa (relative abundance)")
...
fig5a <- plot_bar(phylo_core_genus_rel, x = "Genotype", fill="Genus", colour="Genus")+
  geom_bar(stat="identity")+
  theme_bw()+
  theme(panel.grid = element_blank(), legend.position = "bottom", strip.background = element_blank()) +
  facet_wrap(~Treatment, ncol = 2) +
...
core_disp_pval <- substitute(italic(P[disp])==p, list(p = format(core_anova[[1,5]], digits = 2)))
core_pval <- substitute(italic(P)==p, list(p = format(core_adonis[[1,5]], digits = 2)))


fig5c <- ggplot(data = pseq_core_rel_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
  geom_point(color = "black", size = 4) +
  stat_ellipse()+
  theme_bw()+
  theme(panel.grid = element_blank()) +
  ...
  labs(x = paste0("Axis 1 (", percvar_rel[1], "%)"),
       y = paste0("Axis 2 (", percvar_rel[2], "%)")) +
  annotate("text", x = 0.7, y = -0.21, label = deparse(core_disp_pval), parse = TRUE, size = 4) +
  annotate("text", x = 0.7, y = -0.19, label = deparse(core_pval), parse = TRUE, size = 4) +
  ggtitle("C.   PCoA of core taxa (relative abundance)")
fig5c
...
fig5d <- ggplot(data = pseq_core_rel_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
  geom_point(color = "black", size = 4) +
  stat_ellipse()+
  theme_bw()+
  theme(panel.grid = element_blank()) +
