...
#### **Colleen B. Bove^1\*^, Katharine Greene^1^, Sharla Sugierski^2^, Nicola G. Kriefall^1^, Alexa K. Huzar^1^, Annabel M. Hughes^1^, Koty Sharp^3^, Nicole D. Fogarty^2^, Sarah W. Davies^1\*^**
...
*Repository DOI hosted on Zenodo:* [![DOI](https://zenodo.org/badge/510886822.svg)](https://zenodo.org/badge/latestdoi/510886822)


<br/>

## Microbiome assessment (16S and ITS2) for *A. cervicornis* microplastics project 

This repository contains all code and metadata necessary for the 16S and ITS2 analyses. The code used to assess the gene expression profiles in the manuscript can be found on Sarah Davies' GitHub repository [https://github.com/daviessw/Acer_OAW-Microplastics](https://github.com/daviessw/Acer_OAW-Microplastics).
...
## need this specific update to make collapse_rows() work
...
trt_col <- c("#abd9e9", "#2c7bb6","#fdae61", "#d7191c")
...
### Rename the treatment levels in the metadata file and save this new metadata file on 20 Oct 2022 
# meta_update <- read.csv("Data/Acerv_MP_Metadata.csv")
# levels(meta_update$Treatment) <- c("AMB", "MP", "blank_control", "OAW", "OAW+MP") 
# write.csv(meta_update, "Data/Acerv_MP_Metadata.csv", row.names = FALSE)
...
if(length(grep("Dropbox", wd)) > 0) {
  path_16S <- "/Users/colleen/Dropbox/BU/DaviesLab/ACER_microplastics - in review/16S_fastq" 
} else {
...
#formatting the table for mcmc.otu - requires one first column that's 1 through whatever & has "X" as column name
...
seq_formcmc$X <- nums
seq_formcmc$sample <- samples #3:3274

seq_trim_allinfo <- purgeOutliers(seq_formcmc, count.columns = 3:length(seq_formcmc), sampleZcut = -2.5, otu.cut = 0.0001, zero.cut = 0.02)
# no bad samples identified
# 260 ASVs in 35 samples pass filters (416 ASVs in 19 samples)

#remove sample info
seq_trim <- seq_trim_allinfo[,3:length(seq_trim_allinfo)] 
...
seqtab_trim_rare <- phyloseq(otu_table(seqtab_trim_rare, taxa_are_rows=FALSE), 
                    sample_data(samdf_trim), 
                    tax_table(taxa_clean))
seqtab_trim_rare # 260 taxa remain


## Save the trimmed and rarefied data
save(seqtab_trim_rare, seqtab_trim_rare, samdf_trim, taxa_clean, file = "Data/16s_data/seqtab_trim_rare.Rdata")
...
phylo_trim_rel <- transform_sample_counts(phylo_trim, function(x) x / sum(x))
phylo_trim_rel_df <- data.frame(tax_table(phylo_trim_rel))

trim_rare_rel_phyla <- plot_bar2(phylo_trim_rel, x = "Genotype", fill="Phylum", colour="Phylum")+
  ...
  facet_wrap2( ~ Treatment, scales = "free_y", ncol = 4,  
    strip = strip_themed(background_x = list(element_rect(fill = opacity(trt_col, 0.5)[[1]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[2]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[3]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[4]])) )) +
...
#library(rwantshue)
#scheme <- iwanthue() # recreate with a seed
#cols <- scheme$hex(21)
#
## ... or make custom adjustments:
#color_space <- list(
#	c(0, 360),	# hue range [0,360]
#	c(40, 70),		# chroma range [0,100]
#	c(30, 100))		# lightness range [0,100]
#palette <- scheme$hex(
#	n = 21,
#	force_mode = FALSE,
#	quality = 50,
...
#
## "#b13266" "#6be59b" "#864caf" "#bfe781" "#5852a5" "#9fb140" "#827fe2" "#dec362" "#518dda" "#cd8a3b" "#d589db" "#5faf5f" #"#a0418d" "#3f7a2d" "#e070a5" "#4cd7bf" "#a73235" "#837c28" "#dd6370" "#964e1c" "#e07654"
#
#plot_bar2(phylo_trim_rel, x = "Genotype", fill="Phylum", colour="Phylum")+
#  geom_bar(stat="identity") +
#  theme_bw() +
#  theme(panel.grid = element_blank(), legend.position = "bottom", ) +
...
```{r microshades plot NOT IN PAPER, eval=FALSE, include=FALSE}

#### Playing with microshades! Not using it for this project, but saving this for future reference and to spread the word.
...
mdf_prep <- prep_mdf(phylo_trim_rel)

# Generate a color object for the specified data
color_objs_GP <- create_color_dfs(mdf_prep, selected_groups = c("Planctomycetota", "Proteobacteria", "Bacteroidota", "Cyanobacteria", "Firmicutes") , cvd = TRUE)
...
# add additional colours to the proteo phylum
...
# update legend with extended colours
micro_legend <- custom_legend(new_groups$mdf, new_groups$cdf)

# Plot
microshade_plot <- plot_microshades(new_groups$mdf, new_groups$cdf, x = "Genotype") + 
  scale_y_continuous(labels = scales::percent, expand = expansion(0)) +
  #theme(axis.text.x = element_text(size= 6)) +
  facet_wrap2( ~ Treatment, scales = "free_y", ncol = 2, nrow = 2,
               strip = strip_themed(background_x = list(element_rect(fill = opacity(trt_col, 0.5)[[1]]),
                                                        element_rect(fill = opacity(trt_col, 0.5)[[2]]),
                                                        element_rect(fill = opacity(trt_col, 0.5)[[3]]),
                                                        element_rect(fill = opacity(trt_col, 0.5)[[4]])) )) +
...
plot_grid(microshade_plot, micro_legend,  rel_widths = c(1, .25))
...
phylo_proteo <- subset_taxa(phylo_trim, Phylum %in% "Proteobacteria")
...
proteo_plot <- plot_bar2(phylo_proteo_rel, x = "Genotype", fill = "Order", colour = "Order")+
  ...
  scale_fill_manual(values = rev(colorRampPalette(brewer.pal(8, "Paired"))(24))) +
  scale_colour_manual(values = rev(colorRampPalette(brewer.pal(8, "Paired"))(24))) +
  ylab("Relative abundance") +
...
```{r count aqua abundance to report min max}

## make dataframe from the rel abundance of all ASVs (before selecting any particular taxa)
otu_df <- data.frame(phylo_trim_rel@otu_table)

# min aquarick abundance (ASV 1)
aqua_min <- round((min(otu_df$ASV1) * 100), 1)

# max aquarick abundance
aqua_max <- round((max(otu_df$ASV1) * 100), 1)
...
rick_plot <- plot_bar2(phylo_rick_rel, x = "Genotype", fill = "seqs_ps", colour = "seqs_ps")+
  ...
  scale_fill_manual(values = (colorRampPalette(brewer.pal(8, "Paired"))(5)), labels = rownames(phylo_rick_df)) +
  scale_colour_manual(values = (colorRampPalette(brewer.pal(8, "Paired"))(5)), labels = rownames(phylo_rick_df)) +
  ylab("Relative abundance") +
  facet_wrap2( ~ Treatment, scales = "free_y",  
    strip = strip_themed(background_x = list(element_rect(fill = opacity(trt_col, 0.5)[[1]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[2]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[3]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[4]])) )) +
  ggtitle("Rickettsiales ASVs")
rick_plot
#ggplotly(rick_plot)

```

```{r aqua asv percentages, include=FALSE}

### Calculate the relative abundance of Rickettsiales ASVs of all samples
relabun_aqua <- tax_glom(phylo_trim_rel, taxrank = "Order", NArm = FALSE)
relabun_aqua <- subset_taxa(relabun_aqua, Order %in% "Rickettsiales")

aqua_percent_df <- psmelt(relabun_aqua) %>%
  group_by(Sample, Treatment, Order) %>%
  summarize(mean_abund = round(mean(Abundance, na.rm=TRUE), 2)) 
...
load(file = "Data/16s_data/phylo_trim.Rdata")
phylo_trim # 257 rarefied and trimmed across all 36 samples
...
#rel abun
pseq_rel_ord <- plot_ordination(phylo_trim_rel, ordinate(phylo_trim_rel,"PCoA", "bray"), color = "Treatment", shape = "Genotype", justDF = TRUE) 
eigvec_rel <- ordinate(phylo_trim_rel,"PCoA", "bray")$values["Eigenvalues"]
percvar_rel <- round(100 * eigvec_rel[1:2,]/sum(eigvec_rel), 1)



### Dispersion stats of core taxa 
seq_all <- data.frame(otu_table(phylo_trim_rel)) # pull the otu table from the core object
dist_all <- vegdist(seq_all) # calculate Bray-Curtis distances between samples
samdf_all <- data.frame(sample_data(phylo_trim_rel)) # pull sample data from the core phyloseq object
...
all_gen_anova <- anova(dist_all_gen)
...
ggsave("Figures/Exploratory/16S_PCoA_all.png", height = 5, width = 10, dpi = 600)
 ...
 top20 <- names(sort(taxa_sums(phylo_trim), decreasing=TRUE))[1:20]
 ps.top20 <- transform_sample_counts(phylo_trim, function(OTU) OTU/sum(OTU))
...
plot2 <- plot_bar(ps.top20, x = "Genotype", fill="Genus")+
  ...
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_fill_manual(values = (colorRampPalette(brewer.pal(8, "Paired"))(20))) +
  #scale_colour_manual(values = rev(colorRampPalette(brewer.pal(8, "Paired"))(25))) +
...
## Selecting just Phylum Proteobacteria 
gen_rick <- subset_taxa(phylo_trim, Genus != "MD3-55")
...
simp_noR_plot <- ggplot(alpha_noAq_df_div, aes(x = Treatment, y = InvSimpson, color = Treatment, fill = Treatment, shape = Treatment)) +
  geom_point(alpha = 0.3, position = position_jitter(width = 0.2), size = 2, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
  #scale_shape_manual(values = gen_shape) +
...
shan_noR_plot <- ggplot(alpha_noAq_df_div, aes(x = Treatment, y = Shannon, color = Treatment, fill = Treatment, shape = Treatment)) +
  geom_point(alpha = 0.3, position = position_jitter(width = 0.2), size = 2, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
rich_noR_plot <- ggplot(alpha_noAq_df_div, aes(x = Treatment, y = Observed, color = Treatment, fill = Treatment, shape = Treatment)) +
  geom_point(alpha = 0.3, position = position_jitter(width = 0.2), size = 2, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
even_noR_plot <- ggplot(alpha_noAq_df_div, aes(x = Treatment, y = InvSimpson, color = Treatment, fill = Treatment, shape = Treatment)) +
  geom_point(alpha = 0.3, position = position_jitter(width = 0.2), size = 2, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
## Dispersion by genotype: not significant 
dist_all_gen <- betadisper(dist_all, samdf_all$Genotype)
all_genNoA_anova <- anova(dist_all_gen)
...
beta_noR_plot <- ggarrange(pcoa_noA_plot, dist_noA_plot, common.legend = TRUE, legend = "bottom") + 
...
pseq_core <- core(phylo_trim, detection = 0, prevalence = 0.99)
...
plot_bar2(phylo_core_genus_rel, x = "Genotype", fill="Genus", colour="Genus")+
  ...
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_fill_manual(values = (colorRampPalette(brewer.pal(8, "Paired"))(8))) +
  scale_colour_manual(values = (colorRampPalette(brewer.pal(8, "Paired"))(8))) +
  facet_wrap(~Treatment, ncol = 2) +
...
ps_rare_trim_rel <- transform_sample_counts(phylo_trim, function(x) x / sum(x)) # calc rel abund of the rare core
...
ps_rare_trim_otu <- data.frame(phylo_trim@otu_table)
...
```{r raw counts of its2 reads}

## read in raw counts and rename things
its_rawcounts <- read.csv("Data/ITS2_data/rawcounts.csv", header = FALSE) %>% 
  rename(SampleID = V1, counts = V2) %>%  # rename columns
  mutate(counts = as.numeric(counts), # convert counts to numeric
         SampleID = factor(gsub("_R1_ITS.fastq.gz", "", SampleID))) # remove file extension from samples


## read in the meta data to filter the desired samples (the full raw ITS2 list include counts from another project)
meta_raw <- read.csv("Data/Acerv_MP_Metadata.csv") %>% 
  filter(Treatment != "blank_control") # exclude meg controls

## select for just the microplastic samples
its_rawcounts <- its_rawcounts[its_rawcounts$SampleID %in% meta_raw$sampleID,]



its_raw_df <- data.frame(mean = round(mean(its_rawcounts$counts), 0),
                         max = max(its_rawcounts$counts),
                         min = min(its_rawcounts$counts),
                         sd = round(sd(its_rawcounts$counts), 0))

```

```{r kable of raw count stats}

kable(its_raw_df, caption = "Raw count statistics of ITS2 samples") %>% 
  kable_minimal() %>% 
  kable_styling(font_size = 14, full_width = FALSE)
...
```{r counts post symportal, include=FALSE}

### This portion to calculate count stats was written and run by Annabel Hughes:

library(Rmisc)

# all samples
its_df_total <- cbind(its_df, total = rowSums(its_df)) # create a new column summing up counts for each sample
symp_count_mean <- mean(its_df_total$total) # mean 
symp_count_sd <- sd(its_df_total$total) # std dev
symp_count_max <- max(its_df_total$total) # max counts
symp_count_min <- min(its_df_total$total) # min counts

# all samples - by treatment
merge_all <- merge(its_df_total, meta, by = 0)
summarySE(merge_all, measurevar = "total", groupvars = c("Treatment")) # stats based on counts per treatment
max(merge_all$total)
min(merge_all$total)

```

```{r kable of sym count stats}

# combine stats into a dataframe
its_sym_df <- data.frame(mean = round(mean(its_df_total$total), 0),
                         max = max(its_df_total$total),
                         min = min(its_df_total$total),
                         sd = round(sd(its_df_total$total), 0))

kable(its_sym_df, caption = "Post-SymPortal processing count statistics of ITS2 samples") %>% 
  kable_minimal() %>% 
  kable_styling(font_size = 14, full_width = FALSE)
...
```{r calc rel abund and save phylo objc, fig.height = 4.5, fig.width = 6, fig.align = "center"}
...
plot_bar2(phylo_its2_rel, x = "Genotype", fill = "majority_its2", colour = "majority_its2", size = 0.2)+
  theme_bw() +
    facet_wrap2( ~ Treatment, scales = "free_y", ncol = 2, nrow = 2,  
    strip = strip_themed(background_x = list(element_rect(fill = opacity(trt_col, 0.5)[[1]]),
                          element_rect(fill = opacity(trt_col, 0.5)[[2]]),
  ...
  scale_color_manual("Majority ITS2", values = c("#636363", "#636363")) +
  scale_fill_manual( "Majority ITS2", values = c("#d9d9d9", "#636363")) +
...
plot_bar2(its_rel_trt, x = "Treatment", fill = "majority_its2", colour = "majority_its2")+
  ...
  theme(panel.grid = element_blank(), legend.position = "bottom") +
  labs(x = "", y = "Relative abundance") +
  scale_color_manual("Majority ITS2", values = c("#d9d9d9", "#636363")) +
  scale_fill_manual( "Majority ITS2", values = c("#d9d9d9", "#636363")) +
...
### All coral fragments are dominated by the microalgal symbiont *Symbiodinium fitti*

Raw ITS2 counts before submission to *SymPortal* ranged from `r its_raw_df$min` to `r its_raw_df$max`, with a mean of `r its_raw_df$mean`. After classification by *SymPortal*, ITS2 counts ranged from `r symp_count_min` – `r symp_count_max` per sample across all 36 samples with mean `r round(symp_count_mean, 0)` counts. All coral individuals hosted *Symbiodinium ‘fitti’* (ITS2 type A3) with 44% of individuals (N=16) hosting small background amounts of *Breviolum minutum* (ITS2 type B2) (Figure 2).
...
Cleaned 16S reads before trimming and rarefaction averaged `r format(round(mean_preRare, 0), scientific = FALSE)` ± `r format(round(sd_preRare, 0), scientific = FALSE)` (±SD) per sample across all *A. cervicornis* samples with a minimum of `r format(min(preRare_count), scientific = FALSE)` (Sample `r as.character(track_reads$Sample[track_reads$"Sample ID" == min_sample])`) and a maximum of `r format(max(preRare_count), scientific = FALSE)` (Sample `r as.character(track_reads$Sample[track_reads$"Sample ID" == max_sample])`). Fragments were dominated by taxa from the phylum Proteobacteria (**Figure 5A**), which was dominated by a single ASV (Genus MD3-55; *Ca.* Aquarickettsia) within the order Rickettsiales (**Figure 5B**; see **Supplemental Figure 5**). This dominant ASV was a 100% match to the published 16S rRNA sequence for the putative bacterial parasite “*Ca.* Aquarickettsia rohweri” (hereafter, *Aquarickettsia*) (J. G. Klinges et al. 2019) that is common in *A. cervicornis* across the Caribbean (Williams et al. 2022; Godoy-Vitorino et al. 2017).

Beta diversity analyses did not identify any statistical differences across samples based experimental treatment or genotype (**Figure 5C and Table SS4**). Similarly, alpha diversity was indistinguishable across treatments (**Supplemental Figure S6 and Table S5**). After removing *Aquarickettsia* from all samples to assess background bacterial communities, there were still no differences in alpha and beta diversity across treatments or genotypes (**Supplemental Figure S7 and Table S6**).
...
```{r figure 2, fig.height = 4.5, fig.width = 6}
...
ggsave("Figures/Fig2_ITS2.png", height = 4.5, width = 6, dpi = 600)
ggsave("Figures/Fig2_ITS2.pdf", height = 4.5, width = 6, useDingbats = FALSE)
...
**Figure 5:** *Acropora cervicornis* bacterial (16S) relative abundance across fragments and experimental treatments. Diversity at the phylum level (**A**) depicts dominance of all samples by taxa from Proteobacteria and this (**B**) phylum was dominated by the order Rickettsiales in most samples. Bacterial diversity color was assigned alphabetically, not based on abundance. Beta diversity was visualized through (**C**) multivariate ordination plots (PCoA) of between-sample Bray–Curtis dissimilarity of rarefied ASVs of all taxa and (**D**) distance to treatment centroids. PCoA ellipses depict 95% confidence intervals and p-values indicate significance of treatment and genotype. Treatment is depicted by color in all panels: light blue = ambient conditions (AMB), dark blue = ocean acidification and warming (OAW), orange = microplastics (MP), and dark red = OAW and MP (OAW+MP).
...
```{r fig S1 conditions, fig.align = 'center', fig.height = 5, fig.width = 12}

env_df <- read.csv("Data/Acerv_MP_EnvData.csv", header = TRUE)[1:11] %>% 
  mutate(Tank = factor(Tank),
         Combined.Treatmenr = fct_recode(Combined.Treatmenr, # rename the treatment IDs to match
                                         "AMB" = "AMB Control", 
                                         "MP+" = "AMB MP+", 
                                         "OAW" = "OAW Control", 
                                         "OAW+MP" = "OAW MP+")) 
  
## treatment summary through time 
env_summary <- env_df %>% 
  group_by(Combined.Treatmenr, Date) %>% 
  dplyr::summarise(across(where(is.numeric), list(mean = mean, sd = sd, min = min, max = max), na.rm = TRUE)) %>% 
  dplyr::rename(Treatment = Combined.Treatmenr)


env_plot_df <- env_summary %>% 
  pivot_longer(Temp_mean:pH_max, names_to = "param", values_to = "value") %>% 
  separate(param, c("Param", "Measure")) %>% 
  pivot_wider(names_from = Measure, values_from = value) %>% 
  mutate(Date = as.Date(Date, format = "%m/%d/%y"),
         Param = fct_recode(Param, # rename parameters with units
                                         "DO (ppm)" = "DO", 
                                         "pH (total scale)" = "pH", 
...
figS1 <- ggplot(data = env_plot_df, aes(x = Date, y = mean, colour = Treatment, group = Treatment)) +
  geom_ribbon(aes(ymin = mean - sd, ymax = mean + sd, fill = Treatment), alpha = 0.3, colour = NA) +
  geom_line() +
  theme_bw() +
  facet_wrap(~ Param, scales = "free_y") +
  theme(panel.grid = element_blank(), axis.title = element_blank()) +
  scale_x_date(date_breaks = "3 days", date_labels = "%d %b", limit = c(min(env_plot_df$Date), max(env_plot_df$Date)), expand=c(0,0)) +
  scale_colour_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) 
figS1

## save figure 
ggsave("Figures/Supplemental/FigureS1_WaterQuality.png", height = 5, width = 12, dpi = 600)
ggsave("Figures/Supplemental/FigureS1_WaterQuality.pdf", height = 5, width = 12, useDingbats = FALSE)
...
```{r fig S5 rick ASVs, fig.align = 'center', fig.height = 7, fig.width = 8}

## renaming the rick ASV plot for supps
figS4 <- rick_plot + theme(legend.title = element_blank(), legend.position = "bottom")
figS4

## save figure 
ggsave("Figures/Supplemental/FigureS5_RickettsialesASVs.png", height = 7, width = 8, dpi = 600)
ggsave("Figures/Supplemental/FigureS5_RickettsialesASVs.pdf", height = 7, width = 8, useDingbats = FALSE)
...
```{r fig S5 alpha diversity, fig.align = 'center', fig.height = 7, fig.width = 8}

figS6 <- ggarrange(simp_stat_plot, shan_stat_plot, rich_stat_plot, even_stat_plot, common.legend = TRUE, labels = "AUTO", legend = "right") 
figS6

## save figure 
ggsave("Figures/Supplemental/FigureS6_alpha.png", height = 7, width = 8, dpi = 600)
ggsave("Figures/Supplemental/FigureS6_alpha.pdf", height = 7, width = 8, useDingbats = FALSE)
...
```{r fig S6 no rick diversity, fig.align = 'center', fig.height = 7, fig.width = 8}
...
## remove legend from the beta diversity distance
dist_noA_plot <- dist_noA_plot + theme(legend.position = "none")


## add plots together with layout
figS6 <- simp_noR_plot + shan_noR_plot + rich_noR_plot + even_noR_plot + pcoa_noA_plot + dist_noA_plot + plot_layout(design = layoutS6, guides = "collect") + plot_annotation(tag_levels = 'A')
figS6

## save figure 
ggsave("Figures/Supplemental/FigureS6_roRock_alpha.png", height = 6, width = 16, dpi = 600)
ggsave("Figures/Supplemental/FigureS6_roRock_alpha.pdf", height = 6, width = 16, useDingbats = FALSE)

```

**Supplemental Figure 5:** Alpha (**A-D**) and beta (**E-F**) diversity metrics across all samples in all treatments once Aquarickettsia ASVs were removed. Plots depict (**A**) Simpson index, (**B**) Shannon index, (**C**) species richness, (**D**) Evenness, (**E**) multivariate ordination plots (PCoA) of between-sample Bray–Curtis dissimilarity of rarefied ASVs and (**F**) distance to treatment centroids. PCoA ellipses depict 95% confidence intervals and p-values indicate significance of treatment and genotype. Treatment is depicted by color in all panels: light blue = ambient conditions (AMB), orange = ocean acidification and warming (OAW), dark blue = microplastics (MP), and dark red = OAW and MP (OAW+MP).
...
## Tank summary
tank_summary <- env_df %>% 
  group_by(Combined.Treatmenr, ID) %>% 
  dplyr::summarise(across(where(is.numeric), list(mean = mean, sd = sd, min = min, max = max), na.rm = TRUE)) %>% 
  dplyr::rename(` ` = ID) %>% 
  dplyr::rename(Treatment = Combined.Treatmenr)
  
## Tank summary
treat_summary <- env_df %>% 
  group_by(Combined.Treatmenr) %>% 
  dplyr::summarise(across(where(is.numeric), list(mean = mean, sd = sd, min = min, max = max), na.rm = TRUE)) %>% 
  add_column(Treatment = "Treatment Summary", .before = "Combined.Treatmenr") %>% 
  dplyr::rename(` ` = Combined.Treatmenr)


## Combine them
env_summary <- rbind(data.frame(treat_summary), data.frame(tank_summary))

rep_list <- c("Mean", "SD", "Minimum", "Maximum")


## makrdown version of supp table
kable(env_summary, row.names = FALSE, digits = 2, col.names = c(" ", " ", rep(rep_list, 4))) %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 10, full_width = FALSE) %>% 
  add_header_above(c(" " = 2, "Temperature" = 4, "Dissolved Oxygen" = 4, "Salinity" = 4, "pH" = 4)) %>%
  pack_rows("Treatment", 1, 4) %>%
  pack_rows("Tank", 5, 40) %>%
  column_spec(c(2,5,9,13), border_right = TRUE) %>% 
  kableExtra::collapse_rows(columns = 1, valign = "top") %>%
  save_kable("Tables/TableS1_SWconditions.html")

## save table S1 as PDF and CSV
webshot("Tables/TableS1_SWconditions.html", "Tables/TableS1_SWconditions.pdf")
write.csv(env_summary, "Tables/TableS1_SWconditions.csv")
...
**Table S4:** Statistical model output of beta diversity assessments of all ASVs based on (**A**) treatment and (**B**) genotype.
```{r table S4}

## anova of the betadisper of all ASVs
beta_all_anova <- rbind(all_anova, all_gen_anova)

## table 3A: ANOVA 
kable(beta_all_anova, row.names = TRUE, digits = 2, caption = "A) ANOVA of all ASV dispersion") %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  #column_spec(1, bold = TRUE) %>% 
  column_spec(1, border_right = TRUE) %>% 
  pack_rows("Treatment", 1, 2) %>% 
  pack_rows("Genotype", 3, 4)


## table 3B: PERMANOVA
kable(all_adonis, row.names = TRUE, digits = 2, caption = "B) PERMANOVA of all ASVs") %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  add_indent(c(3, 4)) %>% 
  column_spec(1, border_right = TRUE) %>% 
...
**Table S5:** Statistical model output of alpha diversity assessments of all ASVs.
```{r table S5}

## combine all alpha of all ASVs
simp_sum <- data.frame(metric = c(rep("simpson", 4)), summary(simp_lm)[["coefficients"]])
shan_sum <- data.frame(metric = c(rep("shannon", 4)), summary(shan_lm)[["coefficients"]])
rich_sum <- data.frame(metric = c(rep("richness", 4)), summary(rich_lm)[["coefficients"]])
even_sum <- data.frame(metric = c(rep("evenness", 4)), summary(even_lm)[["coefficients"]])

## combine summaries and remove numerics from rownames
alpha_allASVs <- rbind(simp_sum, shan_sum, rich_sum, even_sum)
alpha_allASVs <- add_column(alpha_allASVs, ` ` = rownames(alpha_allASVs), .before = "metric")
alpha_allASVs$` ` <- gsub("[0-9]", "", alpha_allASVs$` `)


## table 4: all alpha stats
kable(alpha_allASVs[-2], row.names = FALSE, digits = 2) %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  #column_spec(1, bold = TRUE) %>% 
  pack_rows("Simpson", 1, 4) %>% 
  pack_rows("Shannon", 5, 8) %>% 
  pack_rows("Richness", 9, 12) %>% 
  pack_rows("Evenness", 13, 16)
...
```{r table S6 df creation}

## anova of the betadisper of all ASVs
beta_noA_anova <- rbind(all_noA_anova, all_genNoA_anova)
...
## construct models (GLMER is used for data that could not be transformed to meet normal assumptions)
simp_mod_noA <- glmer(InvSimpson ~ Treatment + (1|Genotype), family = Gamma, data = alpha_noAq_df_div)
shan_mod_noA <- lmer(sqrt(Shannon) ~ Treatment + (1|Genotype), data = alpha_noAq_df_div)
rich_mod_noA <- lmer(log(Observed) ~ Treatment + (1|Genotype), data = alpha_noAq_df_div)
even_mod_noA <- glmer(Evenness ~ Treatment + (1|Genotype), family = Gamma, data = alpha_noAq_df_div)



## combine all alpha of all ASVs
simp_noA_sum <- data.frame(metric = c(rep("simpson", 4)), summary(simp_mod_noA)[["coefficients"]]) %>% dplyr::rename("Pvalue" = Pr...z..)
shan_noA_sum <- data.frame(metric = c(rep("shannon", 4)), summary(shan_mod_noA)[["coefficients"]])[-4] %>% dplyr::rename("Pvalue" = Pr...t..)
rich_noA_sum <- data.frame(metric = c(rep("richness", 4)), summary(rich_mod_noA)[["coefficients"]])[-4] %>% dplyr::rename("Pvalue" = Pr...t..)
even_noA_sum <- data.frame(metric = c(rep("evenness", 4)), summary(even_mod_noA)[["coefficients"]]) %>% dplyr::rename("Pvalue" = Pr...z..)

## combine summaries and remove numerics from rownames
alpha_noA <- rbind(simp_noA_sum, shan_noA_sum, rich_noA_sum, even_noA_sum)
alpha_noA <- add_column(alpha_noA, ` ` = rownames(alpha_noA), .before = "metric")
alpha_noA$` ` <- gsub("[0-9]", "", alpha_noA$` `)

```


**Table S6:** Non-Aquarickettsia ASV beta (**A-B**) and alpha (**C**) diversity across samples.
```{r render the tables for S6}

## table 6A: ANOVA of non aqua ASVs
kable(beta_noA_anova, row.names = TRUE, digits = 2, caption = "A) ANOVA of non Aquarickettsia dispersion") %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  #column_spec(1, bold = TRUE) %>% 
  column_spec(1, border_right = TRUE) %>% 
...
## table 6B: PERMANOVA of non aqua ASVs
kable(all_noA_adonis, row.names = TRUE, digits = 2, caption = "B) PERMANOVA of non Aquarickettsia") %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  add_indent(c(3, 4)) %>% 
  column_spec(1, border_right = TRUE) %>% 
...
## table 6C: all alpha stats of non aqua ASVs
kable(alpha_noA[-2], row.names = FALSE, digits = 2, caption = "C) Alpha diversity metrics of non Aquarickettsia") %>% 
  kable_classic_2() %>% 
  kable_styling(font_size = 14, full_width = FALSE) %>% 
  #column_spec(1, bold = TRUE) %>% 
  pack_rows("Simpson", 1, 4) %>% 
