...
simp_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = InvSimpson, color = Treatment, fill = Treatment)) +
  geom_point(aes(shape = Genotype), alpha = 0.5, position = position_jitter(width = 0.2), size = 4, colour = "black") +
...
simp_lm <- lmer(InvSimpson ~ Treatment + (1|Genotype), data = alpha_df_div)

# checking the model
check_normality(simp_lm) # residuals appear normal
check_heteroscedasticity(simp_lm) # error variance is homoscedastic
#check_model(simp_lm) # overview: looks meh, but will proceed
...
simp_stat_plot <- simp_plot +
  annotate("text", x = 1, y = 8, label = deparse(simp_plot_pval), parse = TRUE, size = 4)
...
shan_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = Shannon, color = Treatment, fill = Treatment)) +
  geom_point(aes(shape = Genotype), alpha = 0.5, position = position_jitter(width = 0.2), size = 4, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
shan_lm <- lmer(Shannon ~ Treatment + (1|Genotype), data = alpha_df_div)

# checking the model
check_normality(shan_lm) # residuals appear normal
check_heteroscedasticity(shan_lm) # error variance is homoscedastic
#check_model(shan_lm) # overview: looks meh, but will proceed
...
rich_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = Observed, color = Treatment, fill = Treatment)) +
  geom_point(aes(shape = Genotype), alpha = 0.5, position = position_jitter(width = 0.2), size = 4, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
# the linear mixed-effects model with a random slope of genotype produced a singular fit so removing genotype
rich_lm <- lm(log(Observed) ~ Treatment, data = alpha_df_div)  # need to log transform the richness

# checking the model
check_normality(rich_lm) # residuals not norma;
check_heteroscedasticity(rich_lm) # error variance is homoscedastic
#check_model(rich_lm) # overview: looks meh, but will proceed
...
```{r add stats to richness plot}

## Add the stats to the plot
rich_plot_pval <- substitute(italic(P)==p, list(p = format(rich_pval[[1,3]], digits = 2)))

rich_stat_plot <- rich_plot +
  annotate("text", x = 4, y = 175, label = deparse(rich_plot_pval), parse = TRUE, size = 4)
...
even_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = Evenness, color = Treatment, fill = Treatment)) +
  geom_point(aes(shape = Genotype), alpha = 0.5, position = position_jitter(width = 0.2), size = 4, colour = "black") +
  geom_boxplot(fill = NA, outlier.colour = NA) +
  scale_color_manual(values = trt_col) +
  scale_fill_manual(values = trt_col) +
...
even_lm <- lmer(log(Evenness) ~ Treatment + (1|Genotype), data = alpha_df_div)

# checking the model
check_normality(even_lm) # residuals not norma;
check_heteroscedasticity(even_lm) # heteroscedasticity detected in regular data so log-transforming
#check_model(even_lm) # overview: looks meh, but will proceed
...
```{r add stats to evenness plot}

## Add the stats to the plot
even_plot_pval <- substitute(italic(P)==p, list(p = format(even_pval[[1,3]], digits = 2)))

even_stat_plot <- even_plot +
  annotate("text", x = 1, y = 0.7, label = deparse(even_plot_pval), parse = TRUE, size = 4)
...
ggarrange(simp_stat_plot, shan_stat_plot, rich_stat_plot, even_stat_plot, common.legend = TRUE)
...
pseq_core <- core(phylo_trim_rare, detection = 0, prevalence = 0.70)
pseq_core # 10 core taxa
...
#r emake phyloseq object with just accessory
...
ggsave("Figures/Fig4_ITS2.png", height = 4, width = 4, dpi = 600)
ggsave("Figures/Fig4_ITS2.pdf", height = 4, width = 4, useDingbats = FALSE)

```

**Figure 4.** Relative abundance of major ITS2 types grouped by treatment (n = 4-5 corals per treatment). Light green represents *Symbiodinium* (A3) and dark green represents *Breviolum* (B2).
...
## plot top 20 ASVs from XXX phylo object by order
fig5a <- plot_bar2(phylo_core_genus_rel, x = "Genotype", fill="Genus", colour="Genus")+
  geom_bar(stat="identity")+
  theme_bw()+
  theme(panel.grid = element_blank(), legend.position = "right", strip.background = element_blank()) +
  facet_wrap(~Treatment, ncol = 2) +
  scale_fill_manual(values = colorRampPalette(rev(brewer.pal(8, "PiYG")))(10)) +
  scale_colour_manual(values = colorRampPalette(rev(brewer.pal(8, "PiYG")))(10)) +
  ggtitle("A.   Core microbial taxa")

```

```{r fig 5b shannon diversity}

## Sannon diversity plot from above
fig5b <- shan_plot +
  annotate("text", x = 1.1, y = 3.4, label = deparse(shan_plot_pval), parse = TRUE, size = 3) +
...
fig5c <- rel_core_plot +
  ...
  annotate("text", x = 0.5, y = -0.17, label = deparse(rel_core_trt_pval), parse = TRUE, size = 3) +
  annotate("text", x = 0.5, y = -0.19, label = deparse(rel_core_gen_pval), parse = TRUE, size = 3) 

```

```{r fig 5d accessory PCoA}

fig5d <- rel_acc_plot +
  annotate("text", x = -0.9, y = 0.70, label = deparse(rel_acc_disp_pval), parse = TRUE, size = 3) +
  annotate("text", x = 0.75, y = -0.70, label = deparse(rel_acc_trt_pval), parse = TRUE, size = 3) +
  annotate("text", x = 0.75, y = -0.80, label = deparse(rel_acc_gen_pval), parse = TRUE, size = 3) 

```


```{r full fig 5, fig.height = 8, fig.width = 11}
...
Fig5 <- fig5a + fig5b + fig5c + fig5d + plot_layout(design = layout, guides = "collect")
Fig5

## save figure 5
ggsave("Figures/Fig5_16S_diversity.png", height = 8, width = 11, dpi = 600)
ggsave("Figures/Fig5_16S_diversity.pdf", height = 8, width = 11, useDingbats = FALSE)
