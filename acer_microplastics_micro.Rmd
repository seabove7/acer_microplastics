...
system2("/Users/colleen/Library/Python/3.9/bin/cutadapt", args = "--version")
...
#length(keepers) # 3230, so that means only 42 euks got through above
phylo_cleanest <- prune_taxa(keepers, ps_cleaner) 
# 3230 ASVs in phylo_cleanest
...
# plot the rarefaction curve with the 9404 sample line
...
seq_trim_allinfo <- purgeOutliers(seq_formcmc, count.columns = 3:3232, sampleZcut = -2.5, otu.cut = 0.0001, zero.cut = 0.02)
# no bad samples identified
# 257 ASVs pass filters

#remove sample info
seq_trim <- seq_trim_allinfo[,3:259] 
...
rarecurve(seqtab_trim, step = 20, sample = raremax_trim, col = "blue", cex = 0.6, label = FALSE) 
...
# plot the rarefaction curve with the 6829 sample line
rarecurve(seqtab_trim_rare, step = 20, sample = raremax_trim, col = "blue", cex = 0.6, label = FALSE) 
...
phylo_trim_rare <- phyloseq(otu_table(seqtab_trim_rare, taxa_are_rows=FALSE), 
                    ...
                    tax_table(taxa2))
phylo_trim_rare # 257 taxa remain
...
ggarrange(simp_alpha_plot, shan_alpha_plot)
...
#Shannon
shapiro.test(1/(alpha_df_div$Shannon)) # not normal 
hist(alpha_df_div$Shannon) # ew
hist(log(alpha_df_div$Shannon)) # ew
hist(sqrt(alpha_df_div$Shannon)) # best
hist((alpha_df_div$Shannon)^(1/3)) # ew
hist(1/(alpha_df_div$Shannon)) # ew


library(car)
leveneTest(alpha_df_div$Shannon ~ Treatment, data = alpha_df_div) #fine

a_div <- aov(sqrt(Shannon) ~ Treatment, data = alpha_df_div)
check_model(a_div)
summary(a_div)
...
plot_bar(ps2, fill="Genus")+
  ...
  scale_fill_manual(values = c('#017a4a','#FFCE4E','#3d98d3','#7559a2','#ff363c','#794924','#8cdb5e','#d6d6d6','#fb8c00'))
