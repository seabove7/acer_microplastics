...
track_reads <- cbind(meta[c(1,3,5)], filt_out, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab_nochim))
...
## Save as RDS objects
save(phylo, phylo_sp, file = "Data/16s_data/phylo_objs.Rdata") # save both with and without species level and ASV labeling
...
ps_cleaner # 1607 taxa left, 19 samples
...
euks <- read.csv("Data/16s_data/euk_contam_asvs.csv", header = FALSE) %>% 
  mutate(V1 = gsub("sq", "ASV", V1)) # replace sq with ASV
euks_names <- euks$V1 # make list from sequence names that are eukaryotes

alltaxa <- taxa_names(ps_cleaner) 
#length(alltaxa) # 1607 total 
keepers <- alltaxa[!(alltaxa %in% euks_names)] 
#length(keepers) # 1538, so that means only 69 euks got through above
phylo_cleanest <- prune_taxa(keepers, ps_cleaner) 
# 1538 ASVs in phylo_cleanest

# change taxa names to ASVs
taxa_names(phylo_cleanest) <- paste0("ASV", seq(ntaxa(phylo_cleanest)))
...
#colnames(seqtab_rare) <- paste0("ASV", seq(1, length(colnames(seqtab_rare))))
taxa2 <- cbind(taxa_sp, rownames(taxa_sp)) #retaining raw sequence info before renaming
rownames(taxa2) <- paste0("ASV", seq(1, length(rownames(taxa2))))
...
load(file = "Data/16s_data/phylo_rarefy.Rdata")
...
seq_trim_allinfo <- purgeOutliers(seq_formcmc, count.columns = 3:1540, sampleZcut = -2.5, otu.cut = 0.0001, zero.cut = 0.02)
# no bad samples identified
# 416 ASVs pass filters

#remove sample info
seq_trim <- seq_trim_allinfo[,3:418] 
...
save(phylo_trim, seq_trim, samdf_cleanest, taxa2, file = "Data/16s_data/phylo_trim.Rdata")
...
# plot the rarefaction curve with the 15600 sample line
...
# plot the rarefaction curve with the 6829 sample line
...
phylo_trim_rare <- phyloseq(otu_table(seqtab_trim_rare, taxa_are_rows=FALSE), 
                    ...
                    tax_table(taxa2))
phylo_trim_rare # 416 taxa remain


## Save the trimmed and rarefied data
save(phylo_trim_rare, seqtab_trim_rare, samdf_trim, taxa2, file = "Data/16s_data/phylo_trim_rare.Rdata")
...
alpha_df <- data.frame(estimate_richness(phylo_rare, split = TRUE, measures=c("Shannon", "InvSimpson", "Observed")))
#df <- data.frame(estimate_richness(phylo_cleanest, split=TRUE, measures=c("Shannon","InvSimpson","Observed")))
#df <- data.frame(estimate_richness(phylo_trim, split=TRUE, measures=c("Shannon","InvSimpson","Observed")))
#df <- data.frame(estimate_richness(phylo_rare_trim, split=TRUE, measures=c("Shannon","InvSimpson","Observed")))

alpha_df$sampleID <- rownames(alpha_df)
alpha_df_div <- merge(samdf, alpha_df, by = "sampleID") #add sample data

#shannon diversity divided by species richness
alpha_df_div$Eveness <- alpha_df_div$Shannon/(log(alpha_df_div$Observed))

alpha_df_div_con <- subset(alpha_df_div, Treatment == "AMB Control")
alpha_df_div_mp <- subset(alpha_df_div, Treatment == "AMB MP+")
alpha_df_div_oaw <- subset(alpha_df_div, Treatment == "OAW Control")
alpha_df_div_trt <- subset(alpha_df_div, Treatment == "OAW MP+")
...
simp_alpha_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = InvSimpson, color = Treatment)) +
  geom_jitter(alpha = 0.8, colour = "lightgrey") +
...
shan_alpha_plot <- ggplot(alpha_df_div, aes(x = Treatment, y = Shannon, color = Treatment)) +
  geom_jitter(alpha = 0.8, colour = "lightgrey") +
  geom_boxplot(fill = NA) +
  scale_color_manual(values = trt_col) +
  ylab("Shannon index")+
...
ggarrange(simp_alpha_plot, shan_alpha_plot, common.legend = TRUE)

```


```{r shannon stats, eval=FALSE, include=FALSE}
...
#Shannon
shapiro.test((alpha_df_div$Shannon)) # normal-ish
hist(alpha_df_div$Shannon)


library(car)
leveneTest(alpha_df_div$Shannon ~ Treatment, data = alpha_df_div) #fine

a_div <- aov(Shannon  ~ Treatment + Genotype, data = alpha_df_div)
check_model(a_div)
summary(a_div)
TukeyHSD(a_div) #ns for rarefied, trimmed, or not
...
pseq_core <- core(phylo_trim, detection = 0, prevalence = 0.7)
pseq_core <- core(phylo_trim_rare, detection = 0, prevalence = 0.7)
pseq_core # 11 core taxa
#saving
#core.tax <- data.frame(pseq_core@tax_table)
...
ps_glom <- tax_glom(pseq_core, "Genus")
...
plot_bar(ps2, fill="Genus")+
  ...
  scale_fill_manual(values = c('#017a4a','#FFCE4E','#3d98d3','#7559a2','#ff363c','#8cdb5e','#fb8c00','#794924','#d6d6d6'))



#ggsave(file="core.bar.pdf",width=8)
#not rel abun
plot_ordination(pseq_core,ordinate(pseq_core,"PCoA", "bray"), color="Treatment", shape="Treatment")+
  stat_ellipse()+
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_color_manual(values = trt_col)
...
#rel abun
pseq_core.rel <- transform_sample_counts(pseq_core, function(x) x / sum(x))
plot_ordination(pseq_core.rel,ordinate(pseq_core.rel,"PCoA", "bray"),color="Treatment", shape="Treatment")+
  stat_ellipse()+
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_color_manual(values = trt_col)
...
core.sqs <- tax_table(pseq_core)
core.sqs.ids <- row.names(core.sqs)
core.sqs.ids
ps.rare.trim.rel <- transform_sample_counts(phylo_trim_rare, function(x) x / sum(x))
...
```{r core stats, eval=FALSE, include=FALSE}
seq.core <- data.frame(otu_table(pseq_core))
seq.core <- data.frame(otu_table(pseq_core.rel))
dist.core <- vegdist(seq.core)
samdf.core <- data.frame(sample_data(pseq_core))
...
ps.rare.trim.otu <- data.frame(phylo_trim@otu_table)
...
ps.acc <- phyloseq(otu_table(ps.rare.trim.acc.otu, taxa_are_rows=FALSE), 
                         sample_data(samdf[-37:-38,]), 
                         tax_table(taxa2))
ps.acc # 405 taxa accessory

plot_ordination(ps.acc,ordinate(ps.acc,"PCoA", "bray"), color="Treatment", shape="Treatment")+
  stat_ellipse()+
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_color_manual(values = trt_col)
...
plot(bet.all) 
adonis(seq.acc ~ Treatment, data=samdf.acc, permutations=999)
...
## pull top 20 ASVs 
top20 <- names(sort(taxa_sums(phylo_cleanest), decreasing=TRUE))[1:20]
ps_top20 <- transform_sample_counts(phylo_cleanest, function(OTU) OTU/sum(OTU))
...
full_ord <- ordinate(phylo_trim_rare, "NMDS","bray", autotransform = FALSE, parallel = 48, trymax=10^4, maxit = 10^4)

plot_ordination(phylo_trim_rare, full_ord , color = "Treatment") +  
  scale_colour_manual(values = trt_col)+
  geom_point(size=2) +
  theme_set(theme_bw())+
  stat_ellipse(aes(fill="Treatment"))+
  #theme(legend.position = "none")+
  stat_ellipse(aes(fill="Treatment"))+ theme(text = element_text(family = "Times New Roman",size=12))
...
```{r PIECRUST DO WE NEED, eval=FALSE, include=FALSE}
...
phylo_trim <- phyloseq(otu_table(seqtab.trim, taxa_are_rows=FALSE), 
                         sample_data(samdf), 
                         tax_table(taxa2))
phylo_trim #223 taxa during revision

trim.otu <- as.matrix(phylo_trim@otu_table)
trim.taxa <- data.frame(phylo_trim@tax_table)
...
ggsave("Figures/Supplemental/FigSXX_ITS2_treat.png", height = 4, width = 4, dpi = 600)
ggsave("Figures/Supplemental/FigSXX_ITS2_treat.pdf", height = 4, width = 4, useDingbats = FALSE)
