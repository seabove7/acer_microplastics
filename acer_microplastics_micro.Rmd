...
### New figures for coauthors

```{r ggplotly of barplots}
...
```{r barplots for koty, fig.height = 8, fig.width = 14}

## combine them to send to coauthors:

abundance_16s_plot <- trim_rare_rel_phyla / (proteo_plot | rick_plot)
abundance_16s_plot

## save figure 
ggsave("Figures/Exploratory/16S_abundance.png", height = 8, width = 14, dpi = 600)
ggsave("Figures/Exploratory/16S_abundance.pdf", height = 8, width = 14, useDingbats = FALSE)
...
pcoa_all <- ggplot(data = pseq_rel_ord, aes(x = Axis.1, y = Axis.2, colour = Treatment, fill = Treatment, shape = Genotype, group = Treatment)) +
...
dist_plot <- ggplot(dis, aes(x = Treatment, y = dist, color = Treatment, fill = Treatment)) +
...
# 
# 
# plot2 <- plot_bar2(ps.top20, x = "Genotype", fill="Genus", colour="Genus")+
#   geom_bar(stat="identity")+
#   theme_bw()+
#   theme(panel.grid = element_blank()) +
